...
export const bookmarksApi = {
  ...
  async create(payload: CreateBookmarkRequest): Promise<Bookmark> {
    const res = await api.post(API_ROUTES.bookmarks.create, payload);
    return res.data.data;
  },
  ...
  async checkStatus(targetKind: "paper" | "report", targetId: string): Promise<{ bookmarked: boolean; bookmarkId?: string }> {
    const res = await api.get(API_ROUTES.bookmarks.check, { params: { targetKind, targetId } });
    return res.data.data;
  },
};
